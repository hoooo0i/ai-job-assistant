...
- 提供需要明确录音同意的实时面试辅助：开启麦克风后以一条持续会话实时转写，检测到面试问题时基于 JD 和真实简历证据显示回答结构与关键提示。默认单次最长 120 分钟（可通过环境变量调整），音频不写入文件、岗位档案或下载报告。
- 在内存中生成经过脱敏的 Word 和 PDF 报告，并提供下载和建议复制入口。
- 将岗位报告、投递检查清单、已保存的定制简历及已生成的求职信整理成一个 ZIP 投递材料包。
- 通过统一模型适配层调用 OpenAI，为后续接入本地模型预留边界。

本阶段不包含 RAG、后台隐蔽监听、录取概率预测、数据库或模型训练。
...
- 岗位链接是选填项。链接读取依次尝试招聘平台公开接口、网页 `JobPosting`、Next.js 等内嵌 JSON、常见岗位正文容器和整页正文，并在读取后标明采用方式。
- 针对中国招聘网站增加了内嵌数据和常见岗位容器识别：智联招聘公开详情页可读取完整 JD；猎聘等可读取的公开页会自动尝试。已下线职位不会误导入“相似岗位”。
- 字节跳动校招和社招的具体岗位详情链接优先使用同站公开岗位接口，可回填公司、岗位、城市、类型、职位描述和职位要求。
- 腾讯招聘的具体 `jobdesc.html?postId=...` 链接优先使用同站公开岗位接口。
- 小米招聘的具体 `xiaomi.jobs.f.mioffice.cn/.../position/.../detail` 链接优先使用同站公开岗位接口；`hr.xiaomi.com` 的职位列表页不会被误当成单个 JD。
- Moka 公司招聘门户支持导入地址中包含 `#/job/...` 的具体岗位；列表页会提示用户先打开某个岗位，避免把公司介绍误当成 JD。Moka 公开岗位响应只在内存中解密，不读取账号、简历或投递记录。
- BOSS直聘、拉勾、前程无忧等页面若要求登录、验证码或触发反爬，服务器端无法稳定自动读取，页面会明确提示手动粘贴 JD。
- Greenhouse 链接优先使用其公开 Job Board API；LinkedIn、Ashby 和 Lever 等公开详情页也可读取结构化数据或正文。
- 为防止访问本机服务或下载超大页面，链接导入只接受公网 HTTP/HTTPS 地址，最多跟随 6 次跳转，页面上限为 4 MB。
...
- 实时面试辅助开启麦克风后建立 1 条持续转写会话；识别到问题时每题增加 1 次证据提示调用。停止或重新开始会建立新的转写会话，手动输入只调用 1 次证据提示。
